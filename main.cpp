...
void escolher3NosAleatorios(Solucao s, vector<int>& CL)
{
    int indiceAleatorio;
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        s.sequence.insert(s.sequence.end() - 1, CL[indiceAleatorio]);
...
int main(int argc, char** argv) {
 ...
 //   for(int i = 0; i < 10; i++)
   // {
    Solucao best = ILS(maxIter, maxIterIls, n,data);
    cost += best.valorObj;
    //}
    // calcularValorObj(&best, data);
    //cout << n << " -> " << 1 << endl;
    cout << "Custo de S: " << best.valorObj << endl;
